...
Exceptions:
-----------
- If the image filenames do not follow the expected format, the function may raise a `ValueError` 
    when converting row/column parts to integers.
- If `pytesseract` is not installed or properly configured, OCR will fail.
"""
